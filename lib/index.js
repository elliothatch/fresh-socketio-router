...
//TODO: whitelist route option, to determine what to do with unhandled routes
function FreshSocketIoRouter(router, options) {
	// if no router provided, set up base router with default values
	if(!router) {
		console.warn('WARNING: fresh-socketio-router: FreshSocketIoRouter: no router provided, creating default router');
	...
	app = function(socket, next) {
		...
		socket.onevent = function(packet) {
			...
			handleSocketEvent(router, options, socket, args[0], args[1]);
...
function handleSocketEvent(router, options, socket, route, message) {
	...
	router(req, res, finalHandler(req, res, {env: options.env}));
}
...
function SocketIoRequest (socket, route, message) {
	...
	this.method = message.method && message.method.toUpperCase() || 'GET';
