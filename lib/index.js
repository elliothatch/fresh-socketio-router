...
function FreshSocketIoRouter(router, options) {
	...
	app = function(socket, next) {
		...
		socket.onevent = function(packet) {
			...
			var ack = null;
			if(packet.id !== null) {
				ack = socket.ack(packet.id);
			}
			handleSocketEvent(baseRouter, opts, socket, args[0], args[1], ack);
...
function handleSocketEvent(router, opts, socket, route, message, ack) {
	var req = new SocketIoRequest(socket, route, message);
	var res = new SocketIoResponse(socket, route, ack);
	req.res = res;
	res.req = req;
